...
def write_findings(summary: dict, regimes: pd.DataFrame,
    ...
    lines = [
        ...
        "",
        f"_Data: FRED `DEXINUS`, {summary['history_start']} to {summary['history_end']} "
    ...
    lines += [
        ...
        f"**{fmt_pct(summary['current_vol30_annualized'])}**, at the "
        f"**{summary['current_vol_percentile']:.0f}th percentile** of its own history "
        f"(mean {fmt_pct(summary['mean_annualized_vol'])}).",
